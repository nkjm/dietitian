...
angular.module("dietitian",["ui.bootstrap"]).service("state",function(){this.requestReloadPerson=!1,this.person=null,this.reloadPerson=function(){this.requestReloadPerson=!this.requestReloadPerson}}).service("remoting",function(){this.isRemoting=!1,this.status=null,this.setIsRemoting=function(isRemoting){this.isRemoting=isRemoting},this.setStatus=function(status){this.status=status}}).service("socket",function(){this.connect=function(channel){this.connection=io(location.protocol+"//"+location.host+"/"+channel)}}).service("personDb",function($log,$http,$q){this.dbPrefix=location.protocol+"//"+location.host+"/personDb",this.getPerson=function(lineId){var url=this.dbPrefix+"/person/"+lineId;return $http({url:url,method:"get"})},this.updatePerson=function(person){var url=this.dbPrefix+"/person/"+person.line_id;return $http({url:url,method:"put",data:{person:person}})}}).service("personalHistoryDb",function($log,$http,$q,person){this.dbPrefix=location.protocol+"//"+location.host+"/personalHistoryDb",this.getTodayHistory=function(){var url=this.dbPrefix+"/person/"+person.line_id+"/diet_history/today";return $http({url:url,method:"get"})}});
angular.module("dietitian").controller("rootCtl",function($scope,$log,person,personDb,socket,state){$scope.ui={},$scope.ui.state=state,state.person=person,$scope.setCurrentTab=function(event,tab){event.preventDefault(),$scope.ui.currentTab=tab},socket.connect(person.line_id)});
angular.module("dietitian").controller("personCtl",function($scope,$log,$uibModal,personDb,state,remoting){function getPerson(lineId){$scope.ui.remoting.setIsRemoting(!0),$scope.ui.remoting.setStatus("プロフィールをリロードしています"),personDb.getPerson(state.person.line_id).then(function(response){$scope.ui.remoting.setIsRemoting(!1),state.person=response.data},function(error){$scope.ui.remoting.setIsRemoting(!1),$log.error(error)})}$scope.ui={person:state.person,remoting:angular.copy(remoting)},$scope.state=state,$scope.getSexLabel=function(sex){var sexLabel="";return"male"==sex?sexLabel="男":"female"==sex&&(sexLabel="女"),sexLabel},$scope.openPersonForm=function(person){var m=$uibModal.open({controller:"personFormCtl",templateUrl:"personForm",resolve:{person:angular.copy(state.person)}});m.result.then(function(){state.reloadPerson()})},$scope.$watch("state.person",function(newVal,oldVal){newVal!==oldVal&&($scope.ui.person=newVal)}),$scope.$watch("state.requestReloadPerson",function(newVal,oldVal){newVal!=oldVal&&getPerson(state.person.line_id)}),1==state.person.first_login&&$scope.openPersonForm(state.person)}).controller("personFormCtl",function($scope,$uibModalInstance,$log,person,personDb,remoting){function clearAlert(){$scope.ui.alertList=[]}function pushAlert(alert){$scope.ui.alertList.push(alert)}person.birthday=new Date(1e3*person.birthday),$scope.ui={person:person,birthdayPickerOpen:!1,alertList:[],remoting:angular.copy(remoting)},$scope.toggleBirthdayPicker=function(){$scope.ui.birthdayPickerOpen=!$scope.ui.birthdayPickerOpen},$scope.updatePerson=function(person){clearAlert();var personForUpdate=angular.copy(person);try{personForUpdate.birthday=personForUpdate.birthday.getTime()/1e3}catch(e){return void pushAlert("誕生日が正しく入力されていません。")}delete personForUpdate.requiredCalorie,delete personForUpdate.requiredNutrition,$scope.ui.remoting.setIsRemoting(!0),$scope.ui.remoting.setStatus("プロフィールを更新しています"),personDb.updatePerson(personForUpdate).then(function(){$scope.ui.remoting.setIsRemoting(!1),$uibModalInstance.close()},function(error){$scope.ui.remoting.setIsRemoting(!1),$log.error("Failed to update person."),$log.error(error)})}});
angular.module("dietitian").controller("todayCtl",function($scope,$log,socket,state,personalHistoryDb){function drawTodayNutrition(nutrition){var ctx=document.getElementById("today-nutrition-chart"),options={scale:{ticks:{stepSize:25,beginAtZero:!0,max:100}}},data={labels:["炭水化物","たんぱく質","脂肪","食物繊維"],datasets:[{data:[Math.round(100*nutrition.carb/state.person.requiredNutrition.carb),Math.round(100*nutrition.protein/state.person.requiredNutrition.protein),Math.round(100*nutrition.fat/state.person.requiredNutrition.fat),Math.round(100*nutrition.fiber/state.person.requiredNutrition.fiber)],backgroundColor:["#FF6384","#4BC0C0","#FFCE56","#E7E9ED","#36A2EB"],label:"My dataset"}]};$scope.ui.todayNutritionChart&&$scope.ui.todayNutritionChart.destroy(),$scope.ui.todayNutritionChart=new Chart(ctx,{type:"polarArea",data:data,options:options})}function drawTodayCalorie(requiredCalorie,todayCalorie){var ctx=document.getElementById("today-calorie-chart"),options={responsive:!1};todayCalorie>requiredCalorie&&(requiredCalorie=todayCalorie);var data={labels:["摂取済み","残り"],datasets:[{data:[todayCalorie,requiredCalorie-todayCalorie],backgroundColor:["#FF6384","#DDDDDD"]}]};$scope.ui.todayCalorieChart&&$scope.ui.todayCalorieChart.destroy(),$scope.ui.todayCalorieChart=new Chart(ctx,{type:"doughnut",data:data,options:options})}function registerIoEvent(connection){connection.on("personalHistoryUpdated",function(dietList){$scope.$apply(function(){processDietList(dietList)})})}function processDietList(dietList){angular.forEach(dietList,function(diet,dietKey){$scope.ui.dietListByType[diet.diet_type].push(diet),$scope.ui.todayCalorie+=diet.calorie||0,$scope.ui.todayNutrition.carb+=diet.carb||0,$scope.ui.todayNutrition.protein+=diet.protein||0,$scope.ui.todayNutrition.fat+=diet.fat||0,$scope.ui.todayNutrition.fiber+=diet.fiber||0}),$scope.reloadTodayCalorieChart(),$scope.reloadTodayNutritionChart()}$scope.state=state,$scope.ui={},$scope.ui.todayCalorie=0,$scope.ui.todayCaloriePercentage=0,$scope.ui.todayCalorieToGo=0,$scope.ui.todayNutrition={carb:0,protein:0,fat:0,fiber:0},$scope.ui.dietListByType={breakfast:[],lunch:[],dinner:[]},$scope.ui.reloadTodayCalorieChart=1,$scope.ui.reloadTodayNutritionChart=1,$scope.reloadTodayCalorieChart=function(){$scope.ui.reloadTodayCalorieChart=$scope.ui.reloadTodayCalorieChart*-1},$scope.reloadTodayNutritionChart=function(){$scope.ui.reloadTodayNutritionChart=$scope.ui.reloadTodayNutritionChart*-1},$scope.$watch("ui.reloadTodayCalorieChart",function(newVal,oldVal){newVal!=oldVal&&($scope.ui.todayCaloriePercentage=Math.round(100*$scope.ui.todayCalorie/state.person.requiredCalorie),$scope.ui.todayCalorieToGo=state.person.requiredCalorie-$scope.ui.todayCalorie,drawTodayCalorie(state.person.requiredCalorie,$scope.ui.todayCalorie))}),$scope.$watch("ui.reloadTodayNutritionChart",function(newVal,oldVal){newVal!=oldVal&&drawTodayNutrition($scope.ui.todayNutrition)}),$scope.$watch("state.person.requiredCalorie",function(newVal,oldVal){newVal!=oldVal&&$scope.reloadTodayCalorieChart()}),$scope.$watch("state.person.requiredNutrition",function(newVal,oldVal){newVal!==oldVal&&$scope.reloadTodayNutritionChart()}),personalHistoryDb.getTodayHistory().then(function(response){var dietList=response.data;processDietList(dietList)},function(error){$log.error(error)}),registerIoEvent(socket.connection)}).directive("todayDietReportByType",function($compile){return{restrict:"E",templateUrl:"todayDietReportByType",scope:{dietTypeLabel:"@",dietList:"="}}});
angular.module("dietitian").controller("thisWeekCtl",function($scope,$log,state){function drawThisWeekCalorie(){var ctx=document.getElementById("this_week_calorie"),options={scales:{yAxes:[{ticks:{min:0}}]}},data={labels:["日","月","火","水","木","金","土"],datasets:[{label:"必要カロリー",fill:!1,lineTension:.1,borderColor:"rgba(128,128,128,0.7)",borderCapStyle:"butt",borderDash:[],borderDashOffset:0,borderJoinStyle:"miter",pointBorderColor:"rgba(128,128,128,1)",pointBackgroundColor:"#fff",pointBorderWidth:1,pointRadius:1,pointHitRadius:10,data:[3200,3200,3200,3200,3200,3200,3200],spanGaps:!1},{label:"摂取カロリー",fill:!0,lineTension:.1,backgroundColor:"rgba(151,187,205,0.2)",borderColor:"rgba(151,187,205,1)",borderCapStyle:"butt",borderDash:[],borderDashOffset:0,borderJoinStyle:"miter",pointBorderColor:"rgba(151,187,205,1)",pointBackgroundColor:"#fff",pointBorderWidth:1,pointHoverRadius:5,pointHoverBorderWidth:2,pointRadius:1,pointHitRadius:10,data:[3400,3300,3080,2800,3400,null,null],spanGaps:!1}]};$scope.ui.thisWeekCalorieChart=new Chart(ctx,{type:"line",data:data,options:options})}$scope.ui={},drawThisWeekCalorie()});